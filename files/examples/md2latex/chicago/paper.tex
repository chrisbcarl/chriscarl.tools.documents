...
\addbibresource{C:/Users/chris/src/chriscarl.tools.documents/files/examples/md2latex/chicago/paper.bib}
...
\title{\textbf{Paper: Example}}
\date{February 07, 2026} % \today
% \author{John C. Neu
...
\author{Chris Carl
    \thanks{email: \texttt{chris.carl@sjsu.edu}}
}
...
\maketitle
\coursetitle{SJSU 2025F - XXXX 6969 - Writing Class}
...
If you want a basic citation, just use \lstinline{<>} and include the \lstinline{BibTex} key \autocite{CitekeyArticle}. If you want to add a page number, feel free to do so. \autocite[66]{Citekey-Inproceedings}. You can also add a range of page numbers like so: \autocite[69-99]{CitekeyProceedings}. If you want to say section instead of page, use \autocite[\S 69-99]{CitekeyBook}. Say you want to include a chapter title in the middle of the citation, go do it \autocite[Some Chapter Inside, 66-69]{CitekeyBooklet}.

\subsubsection{Advanced}\label{Advanced}



\paragraph{Inline Bibliography}\label{Inline-Bibliography}

Include inline \(\BibTeX\) code so long as you create a heading with the title ``Bibliography", ``Citations", or ``References". Here is an example in literal text, but know that the Markdown document contained this exact text without the backticks and was swallowed up by the processing tool, and here is me referencing the Masters Thesis \autocite{totally-new-citation}.% <!--will be extracted via tool
...
You need to include packages \lstinline{graphicx} if you want figures.\begin{figure}[htbp]
                \centerline{\includegraphics[width=0.66\linewidth]{image.jpg}}
                \caption{image.jpg}
                \label{image.jpg}
            \end{figure}\begin{figure}[htbp]
                \centerline{\includegraphics[width=0.66\linewidth]{960px-LambdaPlaques.jpg}}
                \caption{image online via \autocite{Aimage}}
                \label{960px-LambdaPlaques.jpg}
            \end{figure}I've got two figures, See Fig. ~\ref{image.jpg} and Fig. ~\ref{960px-LambdaPlaques.jpg}. The first figure used is referenced by local file, the other is url. Both are supported. How they are displayed in the document is all down to typesetting and isn't very controllable.
...
watch faith information May week here empty fake water hard join house soon private safe!
...
You need to include packages \lstinline{tabularx,booktabs} if you want tables. For IEEE, I offer two situations--a single column table like Table ~\ref{tbl-1-col}, but double column tables like Table ~\ref{tbl-2-col} are also supported. Currently any table with \(\text{cols} > 3\) will be set up as a double column table else single column table.\begin{table}
\caption{Single Column Table \autocite{CitekeyPhdthesis}}
\label{tbl-1-col}
...
\bottomrule
\end{tabularx}
\end{table}Tables have a specific referencing mechanism which is entirely custom and not portable. It's simply the best I could do on a shoestring...\begin{table*}
\caption{Two Column Table \autocite{CitekeyUnpublished}}
\label{tbl-2-col}
...
They can be cited using the code- prefix like Listing ~\ref{code-python}, Listing ~\ref{code-java}, Listing ~\ref{code-cpp}.\begin{lstlisting}[language=Python, caption={Listing with Python in it.}, label={code-python}]
def func():
    print('hello world')
...
\end{lstlisting}\begin{lstlisting}[language=Java, caption={Listing with Java in it.}, label={code-java}]
public class Main {
...
\end{lstlisting}\begin{lstlisting}[language=C++, caption={Listing with C++ in it.}, label={code-cpp}]
#include <iostream>
using std::cout;
using std::endl;
