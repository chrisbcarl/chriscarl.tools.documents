...
Double spaced words are auto-corrected to single space. ``Quotations" are adjusted accordingly, normally you'd need a backtick to get the opening quote, and a \lstinline{''} to get the closing. This is handled already. Apostrophe's are also handled.
...
If you want a basic citation, just use \lstinline{<>} and include the \lstinline{BibTex} key \cite{Citekey-Inproceedings}. If you want to add a page number, feel free to do so. \cite[p. 66]{Citekey-Inproceedings}. You can also add a range of page numbers like so: \cite[pp. 69-99]{Citekey-Inproceedings}. If you want to say section instead of page, use \cite[\S 69-99]{Citekey-Inproceedings}. Say you want to include a chapter title in the middle of the citation, go do it \cite[Some Chapter Inside, pp. 66-69]{Citekey-Inproceedings}.
...
\begin{lstlisting}[language=Python, caption={Listing with Python in it.}, label={code-python}, showstringspaces=false]
def func():
    print('hello world')
...
\begin{lstlisting}[language=Java, caption={Listing with Java in it.}, label={code-java}, showstringspaces=false]
public class Main {
...
\begin{lstlisting}[language=C++, caption={Listing with C++ in it.}, label={code-cpp}, showstringspaces=false]
#include <iostream>
using std::cout;
using std::endl;
