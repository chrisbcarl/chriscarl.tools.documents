...
You need to include packages \lstinline{graphicx} if you want figures.\begin{figure}[htbp]
            ...
            \end{figure}I've got two figures, See Fig. \cite{image.jpg} and Fig. \cite{960px-LambdaPlaques.jpg}. The first figure used is referenced by local file, the other is url. Both are supported. How they are displayed in the document is all down to typesetting and isn't very controllable.
...
Inline math is supported like \(y = Ax + b\) as well as \(\forall x \in X\). Or you can make explicit equations like Eq \cite{eq-rm-ilp} and Eq \cite{eq-profit}.
...
You need to include packages \lstinline{tabularx,booktabs} if you want tables. For IEEE, I offer two situations--a single column table like Table \cite{tbl-1-col}, but double column tables like Table \cite{tbl-2-col} are also supported. Currently any table with \(\text{cols} > 3\) will be set up as a double column table else single column table.\begin{table}
...
They can be cited using the code- prefix like Listing \cite{code-python}, Listing \cite{code-java}, Listing \cite{code-cpp}.\begin{lstlisting}[language=Python, caption={Listing with Python in it.}, label={code-python}]
def func():
    print('hello world')
...
\begin{enumerate}
\item Section \cite{Introduction}
\item Section \cite{Basics}
\begin{itemize}
\item Section \cite{Autocorrected}
\item Section \cite{Typesetting}
\item Section \cite{Citations}
\begin{itemize}
\item Section \cite{Advanced}
\begin{itemize}
\item Section \cite{Interdoc-Citations}
\item Section \cite{Math}
\item Section \cite{Figures}
\item Section \cite{Quotes}
\item Section \cite{Tables}
\item Section \cite{Code}
\end{itemize}
\end{itemize}
\end{itemize}
\item Section \cite{Lorem-Ipsum}
\begin{itemize}
\item Section \cite{Lots-of-Lorem}
\end{itemize}
\end{enumerate}
